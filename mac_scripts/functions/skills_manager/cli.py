...
from .lifecycle import parse_lifecycle_command, run_add, run_remove, run_status
...
def _parse_request(action, args):
    if action == "source":
        ...
        needed = bool(
            request and request.action in {"add", "edit"}
...
def _wire_context(
    *, cwd, env, stdin, stdout, stderr, paths, catalog, ui,
    ...
    def answer(message, render=None) -> bool:
        if render is None:
            ui.confirm(message)
        choices = (SelectorItem("Yes", "yes"), SelectorItem("No", "no"))
        result = select_items(
            choices, multiple=False, title=message, render=render)
        return result.kind == "submit" and result.selected == ("yes",)
    def confirm_apply(
        *, install: Sequence[object], remove: Sequence[object],
...
def _dispatch(action, args, request, context) -> int:
    ...
    return operation(
        tokens, all_sources=all_sources, yes=yes, dry_run=dry_run,
        context=context)
...
def run_cli(
    ...
    services: Services | None = None) -> int:
    ...
    def callback(tokens: tuple[str, ...]) -> int:
        if needs_npx and not services.has_command("npx", env=env):
            ...
            return 1
        try:
            paths = services.initialize_config(env=env)
        except Exception:
            ui.error("Could not create config directory.")
            return 1
        try:
            catalog = services.read_config(paths)
    ...
    command = click.Command(
        "skm", callback=callback, params=[parameter], add_help_option=False,
