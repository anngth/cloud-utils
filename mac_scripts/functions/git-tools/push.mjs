...
export async function runPushCommand(_args, context = {}) {
  ...
  if (trackingBranch) {
    ...
    if (separator > 0) {
      ...
      if (trackingBranch.slice(separator + 1) !== currentBranch) {
        ui.status("Tracking branch name differs from local branch", { tone: "warning" });
        ui.detail(`Will push explicitly to ${targetRemote}/${currentBranch}`);
      }
    }
  } else {
    ui.status("No upstream tracking branch configured", { tone: "warning" });
    ui.detail(`Will push explicitly to origin/${currentBranch}`);
  }
  ...
  if (remoteRefResult.status === 0) {
    ...
    if (remoteBranchResult.status === 2) {
      ...
      ui.status(
        pruneResult.status === 0
          ? "Pruned stale remote-tracking refs"
          : "Failed to prune stale remote-tracking refs; continuing with push",
        pruneResult.status === 0 ? undefined : { tone: "warning" },
      );
    } else if (remoteBranchResult.status !== 0) {
      ui.status(`Could not verify whether ${targetRemote}/${currentBranch} exists; continuing with push`, { tone: "warning" });
    }
  }
  ...
  if (pushResult.status !== 0) {
    ...
    if (pushResult.stdout.trim()) ui.errorDetail(pushResult.stdout.trim());
    if (pushResult.stderr.trim()) ui.errorDetail(pushResult.stderr.trim());
    ui.errorDetail("Tip: run 'git remote -v' and verify write permission to the remote");
    return 1;
  }

  if (pushResult.stdout.trim()) ui.detail(pushResult.stdout.trim());
  if (pushResult.stderr.trim()) ui.detail(pushResult.stderr.trim());
  ui.status("Force push successful");
  ui.status("Fetching");
  const fetchResult = await git(["fetch"]);
  ui.status(
    fetchResult.status === 0 ? "Fetch successful" : "Fetch failed",
    fetchResult.status === 0 ? undefined : { tone: "failure" },
  );
  ui.end();
  return 0;
}
