...
test("readSources accepts array and legacy presets while preserving item properties", (t) => {
  const sandbox = makeSandbox(t);
  writeFileSync(sandbox.legacyFile, '{"extra":true,"presets":[{"source":"a/one","label":"A"}]}\n');
  assert.deepEqual(readSources(sandbox.legacyFile), [{ source: "a/one", label: "A" }]);
});

test("readSources rejects malformed and structurally invalid data", (t) => {
  const sandbox = makeSandbox(t);
  for (const value of ["{", "{}", "null", "[null]", '[{"source":1}]']) {
    writeFileSync(sandbox.legacyFile, value);
    assert.throws(() => readSources(sandbox.legacyFile), SourceListError);
    assert.equal(readFileSync(sandbox.legacyFile, "utf8"), value);
  }
});
...
test("atomic write uses the pid temporary name and removes it after success", (t) => {
  const sandbox = makeSandbox(t);
  writeSourcesAtomic(sandbox.legacyFile, [{ source: "a/one" }], { pid: 4242 });
  assert.equal(readFileSync(sandbox.legacyFile, "utf8"), '[\n  {\n    "source": "a/one"\n  }\n]\n');
  assert.equal(existsSync(`${sandbox.legacyFile}.4242.tmp`), false);
  assert.equal(hasSource(readSources(sandbox.legacyFile), "a/one"), true);
});

test("a modifying write drops legacy top-level fields and keeps item fields", (t) => {
  const sandbox = makeSandbox(t);
  writeFileSync(sandbox.legacyFile, '{"label":"legacy","presets":[{"source":"a/one","keep":true}]}\n');
  const result = addSources(readSources(sandbox.legacyFile), ["b/two"]);
  writeSourcesAtomic(sandbox.legacyFile, result.items);
  assert.deepEqual(JSON.parse(readFileSync(sandbox.legacyFile, "utf8")), [
...
test("failed atomic rename keeps original bytes and cleans its own temp file", (t) => {
  const sandbox = makeSandbox(t, { legacyList: [] });
  const before = readFileSync(sandbox.legacyFile, "utf8");
  const fs = {
    ...realFs,
  ...
  assert.throws(
    () => writeSourcesAtomic(sandbox.legacyFile, [{ source: "new/repo" }], { fs, pid: 9876 }),
    /rename failed/,
  );
  assert.equal(readFileSync(sandbox.legacyFile, "utf8"), before);
  assert.equal(existsSync(`${sandbox.legacyFile}.9876.tmp`), false);
});
...
test("serialization failure leaves the original untouched", (t) => {
  ...
  assert.throws(() => writeSourcesAtomic(sandbox.legacyFile, [circular], { pid: 2468 }), TypeError);
  assert.equal(readFileSync(sandbox.legacyFile, "utf8"), before);
  assert.equal(existsSync(`${sandbox.legacyFile}.2468.tmp`), false);
});
...
test("temporary-file write failure leaves the original untouched", (t) => {
  ...
  assert.throws(
    () => writeSourcesAtomic(sandbox.legacyFile, [{ source: "new/repo" }], { fs, pid: 1357 }),
    /write failed/,
  );
  assert.equal(readFileSync(sandbox.legacyFile, "utf8"), before);
  assert.equal(existsSync(`${sandbox.legacyFile}.1357.tmp`), false);
});
