...
import os, signal, sys, termios, threading
...
@dataclass(frozen=True, slots=True)
class SelectorState:
    items: tuple[SelectorItem, ...]; cursor: int; selected: frozenset[int]
@dataclass(frozen=True, slots=True)
class SelectorResult:
    kind: SelectorKind; state: SelectorState; selected: tuple[str, ...]
def create_selector_state(
    items: Iterable[SelectorItem], *, initial: Iterable[str] = ()
) -> SelectorState:
    copied, initial_values = tuple(items), frozenset(initial)
    selected = frozenset(
...
def reduce_selector(
    ...
    elif key == "down": cursor = min(max(0, len(state.items) - 1), cursor + 1)
    elif key == "toggle" and multiple and state.items:
        if cursor in selected: selected.remove(cursor)
        else: selected.add(cursor)
    elif key == "select_all" and multiple: selected.update(range(len(state.items)))
    elif key == "clear" and multiple: selected.clear()
    next_state = replace(state, cursor=cursor, selected=frozenset(selected))
    if key == "cancel": return SelectorResult("cancel", next_state, ())
    if key == "submit":
...
def _prompt_input(source: Input | TextIO | None) -> tuple[Input, bool]:
    return ((source, False) if isinstance(source, Input)
            else (create_input(stdin=source), True))
def _prompt_output(destination: Output | TextIO | None) -> Output:
    ...
    try:
        ...
        is_tty = False
    if not is_tty: return DummyOutput()
    if isinstance(destination, Output): return destination
...
def run_selector(
    ...
    *,
    ...
    reducer: Callable[[SelectorState, str], SelectorResult] | None = None,
) -> SelectorResult:
    state = create_selector_state(items, initial=initial)
    reduce_state = reducer or (
        lambda value, key: reduce_selector(value, key, multiple=multiple)
    )
    bindings = KeyBindings()
    prompt_input, owns_input = _prompt_input(input)
    prompt_output = _prompt_output(output)
    tty_fd: int | None = None; original_attributes: list[object] | None = None
    try:
    ...
    def apply(key: str, event: KeyPressEvent) -> None:
        nonlocal state
        result = reduce_state(state, key)
        state = result.state
        if result.kind == "continue": render_or_exit(event.app)
        else: event.app.exit(result=result)
    def suspend(event: KeyPressEvent) -> None: event.app.suspend_to_background(
        suspend_group=False)
    handlers = (
        (("up", "k"), "up"), (("down", "j"), "down"), ((" ",), "toggle"),
        (("a",), "select_all"), (("c",), "clear"), (("enter",), "submit"),
        (("q", "c-c", "c-d"), "cancel"),
    )
    for keys, action in handlers:
        def handler(event: KeyPressEvent, key: str = action) -> None: apply(key, event)
        for key in keys:
            bindings.add(key)(handler)
    bindings.add("c-z")(suspend)
    received: list[int] = []
    def after_render(app: Application[SelectorResult]) -> None:
        if app.is_running and not app.is_done: render_or_exit(app)
        ...
        try:
            ...
            try:
                ...
                if owns_input:
                    prompt_input.close()
    terminating = next(
        (value for value in received if value in (signal.SIGTERM, signal.SIGHUP)),
        None,
    )
